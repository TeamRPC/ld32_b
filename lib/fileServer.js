...
function getAsset(req, res) {
    ...
    // serve user sounds
    if (/^snd_/.test(asset)) {
        var p = path.join(__dirname, '../usersounds', asset);
    ...
    // serve our asset sounds
    else if (/^[0-9]+_/.test(asset)) {
        ...
        switch (asset) {
            ...
            case 'introduction.wav':
                ...
                break;
            case 'beep.wav':
                res.sendFile('c_beep.wav', options);
                break;
            case 'test.wav':
...
function downloadRecording(recordingUrl, cb) {
    var tmpDir = path.join(__dirname, '..', 'usersounds');
    var filename = recordingUrl.replace(/^.*[\\\/]/, ''); // get just the 'filename.wav' from the url 
    var saveFile = path.join(tmpDir, filename);
    console.log('saveFile,tmpDir-- ', saveFile, ' ', tmpDir);
